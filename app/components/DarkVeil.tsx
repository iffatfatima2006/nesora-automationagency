...
/* 
  DarkVeil Component
  Renders a CPPN (Compositional Pattern Producing Network) shader effect.
*/
...
export default function DarkVeil({
    ...
    useEffect(() => {
        ...
        const init = async () => {
            // Delay to ensure container is ready and help debounce rapid re-mounts
            await new Promise(resolve => setTimeout(resolve, 10));

            if (!isMounted || !currentContainer) return;
            if (!document.body.contains(currentContainer)) return;

            // Cleanup partials if needed
            ...
            try {
                ...
                console.warn("DarkVeil: Failed to create WebGL context", e);
                return;
            }
            ...
            const resize = () => {
                ...
                // OGL's setSize overrides canvas CSS dimensions — force back to fill container
                const canvas = gl.canvas as HTMLCanvasElement;
                canvas.style.width = '100%';
                canvas.style.height = '100%';
            };
